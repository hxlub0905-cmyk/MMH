...
class _LoadWorker(QThread):
    progress = pyqtSignal(int, int, str)   # dataset_cur, dataset_total, msg
    ...
    def run(self) -> None:
        try:
            ...
            for i, entry in enumerate(self._entries):
                def _phase(msg: str, _i: int = i, _t: int = total) -> None:
                    self.progress.emit(_i, _t,
                                       f"[{_i+1}/{_t}] {entry.name}  —  {msg}")

                df, parsed = load_dataset(entry, phase_cb=_phase)
                dfs.append(df)
