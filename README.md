...
- **批次結果持久化**：每次 Batch 執行後自動儲存至 `~/.mmh/runs/`；支援歷史記錄瀏覽、載入舊結果至 Report，以及刪除紀錄
...
- **八工作區 UI**：Browse → Recipe → Validate → Measure → Batch → Review → Report → History 完整作業流程
...
| Phase C | 部分完成（2026-04-23） | Batch 即時 Overlay 輸出、TC 路徑向量化（4–5×）；Worker 上限保護、X-CD 標注修正待完成 |
